...
const AlatSatuanPage = () => {
  ...
  useEffect(() => {
    localStorage.setItem('alatSatuanProducts', JSON.stringify(products));
  }, [products]);
  ...
  const validateForm = () => {
    ...
    // Gambar wajib diunggah hanya jika ini produk baru ATAU jika sedang edit tapi tidak ada gambar lama DAN tidak ada file baru
  ...
  const handleSubmit = (e) => {
    ...
    setSubmitting(true); // Aktifkan state submitting
    try {
      const newProduct = {
      ...
      setSubmitting(false); // Nonaktifkan state submitting
    }
  };
  ...
  const handleEdit = (product) => {
    setFormData({
      ...
      // Jika gambar Base64, kosongkan input file (agar bisa upload ulang) dan tampilkan preview
      // Jika gambar sudah URL (misal dari data dummy atau di masa depan dari cloud), biarkan di 'gambar'
      gambar: product.gambar && product.gambar.startsWith('data:') ? '' : product.gambar,
      gambarFile: null, // Reset file input
      harga: product.harga,
      deskripsi: product.deskripsi,
    });
    setImagePreview(product.gambar); // Tampilkan pratinjau gambar yang ada
    setErrors({}); // Hapus error saat memulai edit
    setErrorMessage(''); // Hapus pesan error umum
  };

  // Handle hapus produk
  const handleDelete = (id) => {
    setErrorMessage(''); // Reset pesan error
    // Menggunakan window.confirm sementara, di aplikasi nyata gunakan modal kustom
    if (window.confirm('Apakah Anda yakin ingin menghapus produk ini?')) {
      setDeleting(true); // Aktifkan state deleting
      try {
        ...
        setDeleting(false); // Nonaktifkan state deleting
      }
    }
  };
  ...
  return (
    <div className="p-4 sm:p-8">
      <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-8">Manajemen Alat Satuan</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-lg shadow-md lg:h-fit lg:sticky lg:top-4">
          <h2 className="text-xl font-semibold text-gray-800 mb-6">
            {formData.id ? 'Form Edit Produk' : 'Form Tambah Produk'}
          </h2>
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold text-gray-800 mb-6">Tabel Daftar Alat Satuan</h2>
          {products.length === 0 ? (
            <p className="text-gray-500 text-center">Belum ada produk alat satuan yang ditambahkan.</p>
          ) : (
